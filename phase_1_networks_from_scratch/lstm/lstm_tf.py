...
***NOTE***
This program is pretty much identical to ../rnn/rnn_tf.py. The only real difference is that BasicRNNCell
  has been swaped out with BasicLSTMCell. Easy peasy!
"""
...
class LSTM(object):
    def __init__(self, input_dim, hidden_dim=100, backprop_clip=4, learning_rate=0.005):
        ...
        input = tf.unstack(input, num=self.bptt_clip, axis=1)  # unroll examples into clipped list of tensors for bp truncation
        ...
        outputs, state = tf.nn.rnn(lstm_cell, input, initial_state=initial_state)
